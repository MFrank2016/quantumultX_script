...
console.log("111🎉微博cookie脚本开始运行")
if ($request && $request.method != 'OPTIONS' && $request.url.match(/cardlist/)){
  $.msg("测试1", "🎉获取超话签到链接成功", `开始运行`)
  const listurl = $request.url
  const listheaders = JSON.stringify($request.headers)
  if ($.getdata(tokenurl) == undefined || $.getdata(tokenurl) == "") {
    $.msg("测试2", "🎉获取超话签到链接成功", `开始运行`)
    console.log(listurl)
...
} else if ($request && $request.method != 'OPTIONS' && $request.url.match(/active\_fcheckin/) && $request.url.match(/page\/button/)){
  $.msg("测试4", "🎉获取超话签到链接成功", `开始运行`)
